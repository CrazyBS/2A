...
/* ** */transformedResult.put(NdwMember.MEMBER_CONFIDENTIALITY_CODE, StringUtils.isNotEmpty(data.get(ConsistentFields.IS_BLU.toString())) ? "BLU" : StringUtils.isNotEmpty(data.get(ConsistentFields.IS_PHI.toString())) ? "PHI" : "NON");
        ...
        transformedResult.put(NdwMember.MEMBER_PRIMARY_STREET_ADDRESS_2, null == data.get(ConfidentialAddress.ENAD_ADDR1.toString()) ? data.get(SubscriberAddress.SBAD_ADDR2.toString()) : data.get(ConfidentialAddress.ENAD_ADDR1.toString()));
